...
{
    int result = usb_register(&secbulk_driver);
    if(result)
        printk(KERN_ERR "secbulk:usb_register failed: %d. --hui\n", result);
    else
        printk(KERN_INFO "secbulk:secbulk loaded seccessed. --hui\n");
    return result;
}
...
static void __exit secbulk_exit(void)
{
    ...
    printk(KERN_INFO "secbulk:secbulk unloaded seccessed. --hui\n");
}

module_init(secbulk_init);
module_exit(secbulk_exit);
MODULE_LICENSE("GPL"); // 可以删掉
